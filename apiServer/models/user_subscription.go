...
func (u *UserSubscription) GetUserSubscriptions(userID string) ([]*UserSubscription, error) {
	...
	var records []*Record
	sql := fmt.Sprintf("select user_subscriptions.user_subscription_id,user_subscriptions.user_id,user_subscriptions.started_at,user_subscriptions.created_at,user_subscriptions.updated_at,subscriptions.*,icons.* from user_subscriptions join subscriptions on user_subscriptions.subscription_id = subscriptions.subscription_id join icons on subscriptions.icon_id = icons.icon_id where user_subscriptions.user_id = '%s' ORDER BY user_subscriptions.updated_at asc", userID)
