...
func (p PushNotification) sendMessage(userSubscriptionID string) func(context.Context, string) error {
	return func(ctx context.Context, token string) error {
		...
		p.Metrics.Counter(p.Namespace+"_Send_Message_Info", map[string]string{"UserSubscriptionID": userSubscriptionID, "Title": m.Title, "Body": m.Body, "PushToken": token}).Inc()
